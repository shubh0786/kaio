import { useMemo, useEffect } from 'react';
import { ACCENT, GOLD, CONFETTI_COLORS } from './types';

export const AuroraBackground = () => null;
export const IceCrystals = () => null;

export const ThermometerHero = ({ size = 150, className = '' }: { size?: number; className?: string }) => (
  <svg viewBox="0 0 80 160" width={size * 0.5} height={size} className={className}>
    <defs>
      <linearGradient id="mercury" x1="0" y1="1" x2="0" y2="0">
        <stop offset="0%" stopColor="#14b8a6" /><stop offset="50%" stopColor="#0a5e5e" /><stop offset="100%" stopColor="#001a3f" />
      </linearGradient>
      <linearGradient id="thermBody" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0%" stopColor="rgba(10,94,94,0.12)" /><stop offset="100%" stopColor="rgba(20,184,166,0.08)" />
      </linearGradient>
      <filter id="mercuryGlow"><feGaussianBlur stdDeviation="3" result="b" /><feMerge><feMergeNode in="b" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
    </defs>
    <rect x="28" y="10" width="24" height="108" rx="12" fill="url(#thermBody)" stroke="rgba(10,94,94,0.2)" strokeWidth="1" />
    <circle cx="40" cy="130" r="18" fill="url(#thermBody)" stroke="rgba(10,94,94,0.2)" strokeWidth="1" />
    <circle cx="40" cy="130" r="12" fill="url(#mercury)" filter="url(#mercuryGlow)" opacity="0.85">
      <animate attributeName="r" values="11;13;11" dur="2.5s" repeatCount="indefinite" />
    </circle>
    <rect x="36" y="42" width="8" height="80" rx="4" fill="url(#mercury)" opacity="0.75">
      <animate attributeName="y" values="62;42;55;42" dur="5s" repeatCount="indefinite" />
      <animate attributeName="height" values="60;80;67;80" dur="5s" repeatCount="indefinite" />
    </rect>
    {[30, 50, 70, 90, 110].map(y => <line key={y} x1="23" y1={y} x2="28" y2={y} stroke="rgba(10,94,94,0.2)" strokeWidth="0.8" />)}
  </svg>
);

export const FridgeIllustration = ({ size = 130, className = '' }: { size?: number; className?: string }) => (
  <svg viewBox="0 0 100 140" width={size} height={size * 1.4} className={className}>
    <defs>
      <linearGradient id="tl-fbody" x1="0" y1="0" x2="0.3" y2="1"><stop offset="0%" stopColor="#c5d3e6" /><stop offset="100%" stopColor="#a8b8cf" /></linearGradient>
      <linearGradient id="tl-ffreezer" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0a5e5e" /><stop offset="100%" stopColor="#001a3f" /></linearGradient>
      <linearGradient id="tl-fglass" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="rgba(255,255,255,0.5)" /><stop offset="50%" stopColor="rgba(255,255,255,0.15)" /><stop offset="100%" stopColor="rgba(255,255,255,0.3)" /></linearGradient>
      <filter id="tl-fshadow"><feDropShadow dx="0" dy="5" stdDeviation="6" floodColor="#0a5e5e" floodOpacity="0.15" /></filter>
    </defs>
    <rect x="12" y="5" width="76" height="130" rx="8" fill="url(#tl-fbody)" filter="url(#tl-fshadow)" stroke="#6b7fa0" strokeWidth="1.2" />
    <rect x="12" y="5" width="76" height="40" rx="8" fill="url(#tl-ffreezer)" stroke="#0a5e5e" strokeWidth="1" />
    <rect x="16" y="50" width="68" height="80" rx="5" fill="url(#tl-fglass)" />
    <rect x="76" y="20" width="3" height="14" rx="1.5" fill="#14b8a6" opacity="0.7" />
    <rect x="76" y="65" width="3" height="28" rx="1.5" fill="#14b8a6" opacity="0.5" />
    <line x1="18" y1="82" x2="82" y2="82" stroke="#7a8eaa" strokeWidth="0.5" opacity="0.3" />
    <line x1="18" y1="110" x2="82" y2="110" stroke="#7a8eaa" strokeWidth="0.5" opacity="0.3" />
    <rect x="22" y="55" width="14" height="22" rx="4" fill="#0a5e5e" opacity="0.2" />
    <rect x="40" y="60" width="10" height="17" rx="3" fill="#14b8a6" opacity="0.25" />
    <circle cx="36" cy="96" r="7" fill="#86efac" opacity="0.3" />
    <circle cx="58" cy="92" r="5.5" fill="#67e8f9" opacity="0.25" />
    <rect x="26" y="115" width="16" height="13" rx="3" fill="#14b8a6" opacity="0.2" />
    <rect x="50" y="117" width="12" height="11" rx="3" fill="#0a5e5e" opacity="0.15" />
    <circle cx="30" cy="24" r="2.5" fill="white" opacity="0.35" /><circle cx="50" cy="18" r="1.5" fill="white" opacity="0.25" />
    <circle cx="66" cy="27" r="2" fill="#14b8a6" opacity="0.3" /><circle cx="43" cy="30" r="1" fill="white" opacity="0.2" />
  </svg>
);

export const ColdVapor = () => (
  <div className="flex justify-center gap-3 pointer-events-none">
    {[0, 1, 2, 3, 4].map(i => (
      <div key={i} className="vapor-particle" style={{ animationDelay: `${i * 0.5}s` }}>
        <div className="w-10 h-2 rounded-full blur-sm" style={{ background: 'rgba(10,94,94,0.08)' }} />
      </div>
    ))}
  </div>
);

export const SuccessCheck = ({ size = 110 }: { size?: number }) => (
  <div className="relative" style={{ width: size, height: size }}>
    <div className="absolute inset-0 rounded-full animate-burst-1" style={{ border: `3px solid rgba(10,94,94,0.2)` }} />
    <div className="absolute inset-0 rounded-full animate-burst-2" style={{ border: `2px solid ${GOLD}40` }} />
    <svg viewBox="0 0 100 100" width={size} height={size}>
      <circle cx="50" cy="50" r="38" fill="none" stroke={ACCENT} strokeWidth="3" className="animate-draw-circle" />
      <path d="M30 52 L44 66 L72 34" fill="none" stroke={GOLD} strokeWidth="4.5" strokeLinecap="round" strokeLinejoin="round" className="animate-draw-check" />
    </svg>
  </div>
);

function seededRandom(seed: number) {
  const x = Math.sin(seed * 9301 + 49297) * 233280;
  return x - Math.floor(x);
}

export const CelebrationOverlay = ({ onDone }: { onDone: () => void }) => {
  const particles = useMemo(() =>
    Array.from({ length: 36 }, (_, i) => {
      const rad = ((i / 36) * 360 * Math.PI) / 180;
      const dist = 90 + seededRandom(i) * 180;
      return {
        id: i,
        color: CONFETTI_COLORS[i % CONFETTI_COLORS.length],
        tx: Math.cos(rad) * dist,
        ty: Math.sin(rad) * dist,
        size: 5 + seededRandom(i + 100) * 7,
        delay: seededRandom(i + 200) * 250,
        rot: seededRandom(i + 300) * 720,
      };
    }), []);
  useEffect(() => { const t = setTimeout(onDone, 3200); return () => clearTimeout(t); }, [onDone]);
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 animate-fade-in pointer-events-none">
      <div className="relative">
        {particles.map(p => (
          <div key={p.id} className="absolute confetti-particle rounded-sm"
            style={{ width: p.size, height: p.size * 0.55, background: p.color, left: '50%', top: '50%',
              '--tx': `${p.tx}px`, '--ty': `${p.ty}px`, '--rot': `${p.rot}deg`, animationDelay: `${p.delay}ms`,
            } as React.CSSProperties} />
        ))}
        <div className="animate-pop-in"><SuccessCheck size={110} /></div>
      </div>
    </div>
  );
};
