...
import { useTempLog } from '../useTempLog';
        ...
        <div className="card rounded-2xl p-4 mb-4 shadow-sm">
          ...
          <div className="w-full rounded-full h-2.5 overflow-hidden" style={{ background: 'var(--bg-alt)' }}>
            <div className="h-full rounded-full transition-all duration-500 ease-out"
              style={{ width: `${(dailyProgress / Math.max(units.length, 1)) * 100}%`, background: dailyProgress === units.length ? '#34d399' : `linear-gradient(90deg, ${ACCENT}, #0d7a7a)` }} />
          </div>
          {units.length > 1 && (
          ...
          <div className="mt-4 card rounded-2xl p-4 shadow-sm">
            ...
            <div className="flex flex-wrap gap-1.5">
              {units.map(u => { const t = getTemp(u.id, selectedDate); const s = getTempStatus(u, t); return (
                <span key={u.id} className={`text-xs px-2.5 py-1 rounded-full font-semibold ${s === 'warn' ? 'badge-warn' : ''}`}
                  style={s !== 'warn' ? { background: t ? 'rgba(10,94,94,0.08)' : 'var(--bg-alt)', color: t ? ACCENT : 'var(--text-muted)' } : undefined}>
                  {u.name}: {t ? `${t}°C` : '—'}
                </span>
              ); })}
            </div>
