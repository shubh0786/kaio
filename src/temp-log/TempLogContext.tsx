...
function migrateUnits(raw: unknown[]): Unit[] {
  return raw.map((u) => {
    const item = u as Partial<Unit> & { id: string; name: string };
    return { id: item.id, name: item.name, minTemp: item.minTemp ?? 0, maxTemp: item.maxTemp ?? 5 };
  });
}
...
const TempLogCtx = createContext<TempLogContextType>(null!);
export { TempLogCtx };
...
export function TempLogProvider({ recorder, children }: Props) {
  ...
  const exportExcel = useCallback(async (fromDate: string, toDate: string, signature?: string) => {
    ...
    for (let wi = 0; wi < weeks.length; wi++) {
      ...
      titleCell.value = 'Kaio — Temperature Checks';
  ...
  const exportPDF = useCallback(async (fromDate: string, toDate: string, signature?: string) => {
    try {
      ...
      const NAVY_C: [number, number, number] = [10, 94, 94];
      const NAVY_LIGHT: [number, number, number] = [212, 223, 240];
      const GOLD_PDF: [number, number, number] = [20, 184, 166];
      ...
      const drawWeekTable = (weekDays: Date[], startY: number) => {
        ...
        return lastAutoTableY(doc, tableY + 80);
      };
      ...
      if (actions.length) {
        ...
        curY = lastAutoTableY(doc, curY + 20);
      }
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text('Kaio — Temperature Log', 14, 291);
