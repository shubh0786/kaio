...
const SUPPLIERS_KEY = STORAGE_KEYS.suppliers;
const DELIVERIES_KEY = STORAGE_KEYS.deliveries;
...
export default function SuppliersDeliveries({ recorder }: { recorder: string }) {
  const [suppliers, setSuppliers] = useState<Supplier[]>(() => load(SUPPLIERS_KEY, []));
  const [deliveries, setDeliveries] = useState<DeliveryRecord[]>(() => load(DELIVERIES_KEY, []));
  ...
  const exportSuppliersPDF = async () => {
    ...
    const doc = new jsPDF();
    addPdfTitle(doc, 'Approved Supplier List');

    autoTable(doc, {
      startY: 32,
      head: [['Supplier', 'MPI Reg #', 'Type', 'Verified', 'Contact', 'Phone', 'Products', 'Approved']],
          ...
          <div className="flex items-center justify-between">
            ...
            {suppliers.length > 0 && (
              <button type="button" onClick={exportSuppliersPDF}
                ...
                title="Export supplier list as PDF">
                ...
                <div key={s.id} className="card rounded-2xl overflow-hidden shadow-sm">
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-0 space-y-1" style={{ borderTop: '1px solid var(--border)' }}>
                      ...
                      <DetailRow label="Verified" value={s.registrationVerified ? 'Verified' : 'Not Verified'} icon={s.registrationVerified ? <ShieldCheck size={13} className="text-green-500" /> : <AlertTriangle size={13} className="text-amber-500" />} />
                      ...
                      <div className="flex gap-2 mt-3 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
                        ...
                        <button type="button" onClick={() => deleteSupplier(s.id)} className="min-h-[44px] px-4 rounded-xl text-red-500 hover-danger flex items-center justify-center gap-2 text-sm font-medium">
                ...
                <div key={d.id} className="card rounded-2xl overflow-hidden shadow-sm">
                  <button type="button" onClick={() => setExpandedId(isExpanded ? null : d.id)}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${d.accepted ? 'badge-success' : 'badge-danger'}`}>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-0 space-y-1" style={{ borderTop: '1px solid var(--border)' }}>
                      ...
                      <div className="flex justify-end mt-3 pt-2" style={{ borderTop: '1px solid var(--border)' }}>
                        <button type="button" onClick={() => deleteDelivery(d.id)} className="min-h-[44px] px-4 rounded-xl text-red-500 hover-danger flex items-center justify-center gap-2 text-sm font-medium">
                          <Trash2 size={14} /> Delete
                        </button>
                      </div>
                    </div>
        ...
        <>
          ...
          <div className="sheet p-6">
            ...
            <div className="space-y-3">
              ...
              <CheckItem label="Registration Verified against MPI Register" checked={supplierForm.registrationVerified} onChange={v => setSupplierForm({ ...supplierForm, registrationVerified: v })} />
              <a href="https://registers.mpi.govt.nz/FoodActOperators" target="_blank" rel="noopener noreferrer"
                className="btn-outline min-h-[44px] px-4 rounded-xl text-sm font-medium flex items-center justify-center gap-2 w-full" style={{ color: 'var(--navy)', textDecoration: 'none' }}>
            ...
            <div className="flex gap-2 justify-end mt-6">
              {editingSupplier && (
                <button type="button" onClick={() => deleteSupplier(editingSupplier.id)} className="min-h-[44px] px-4 rounded-xl text-red-600 hover-danger flex items-center gap-2 text-sm font-medium">
                  <Trash2 size={14} /> Delete
                </button>
              )}
