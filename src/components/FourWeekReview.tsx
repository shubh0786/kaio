...
export default function FourWeekReview({ recorder }: { recorder: string }) {
  const [reviews, setReviews] = useState<Review[]>(() => load(STORAGE_KEY, []));
            ...
            <div key={r.id} className="card rounded-2xl p-4">
              ...
              {isExpanded && (
                <div className="mt-3 pt-3 border-t space-y-2" style={{ borderColor: 'var(--border)' }}>
                  ...
                  {r.councilApproval && (
                    <div className="mt-2 p-3 rounded-xl badge-warn" style={{ border: '1px solid currentColor', borderColor: 'rgba(245, 158, 11, 0.3)' }}>
                      <p className="text-sm font-medium">Council approval required</p>
                      {r.councilNotes && <p className="text-xs mt-1" style={{ opacity: 0.85 }}>{r.councilNotes}</p>}
                    </div>
                  )}
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteReview(r.id); }}
                    className="w-full py-3 min-h-[44px] rounded-xl text-sm font-semibold text-red-500 bg-danger-light active:scale-[0.97] transition-all mt-2"
                  >
                    Delete Review
                  </button>
                </div>
