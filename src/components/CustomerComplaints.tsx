...
const STORAGE_KEY = STORAGE_KEYS.complaints;

export default function CustomerComplaints({ recorder }: { recorder: string }) {
  const [complaints, setComplaints] = useState<Complaint[]>(() => load(STORAGE_KEY, []));
              ...
              {isExpanded && (
                <div className="mt-3 pt-3 border-t space-y-3" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <button
                    ...
                    className="w-full py-3 min-h-[44px] rounded-xl text-sm font-semibold text-red-500 bg-danger-light active:scale-[0.97] transition-all"
                  >
                    Delete Complaint
                  </button>
                </div>
