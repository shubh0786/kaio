...
const CLEANING_KEY = STORAGE_KEYS.cleaningLog;
const MAINTENANCE_KEY = STORAGE_KEYS.maintenanceLog;

export default function CleaningMaintenance({ recorder }: { recorder: string }) {
  const [tab, setTab] = useState<Tab>('cleaning');
  const [cleaningEntries, setCleaningEntries] = useState<CleaningEntry[]>(() => load(CLEANING_KEY, []));
  const [maintenanceEntries, setMaintenanceEntries] = useState<MaintenanceEntry[]>(() => load(MAINTENANCE_KEY, []));
